...
class GFT2Measurement(Measurement1D):
    ...
    def __init__(self, ge_info, ef_info, delays, detune=0, echotype=ECHO_NONE, necho=1, double_freq=False, laser_power = None, **kwargs):
        ...
        super(GFT2Measurement, self).__init__(len(delays), infos=(ge_info, ef_info), **kwargs)
...
#        self.data.set_attrs(laser_power =self.laser_power)
    ...
    def generate(self):
        ...
        for i, dt in enumerate(self.delays):
            ...
            s.append(Pad(r_ef(np.pi, 0), 250, PAD_RIGHT))
            s.append(Pad(r_ge(np.pi/2, angle), 250, PAD_RIGHT))
