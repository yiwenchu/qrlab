...
class T2Measurement_QP(Measurement1D):

    def __init__(self, qubit_info, delays, QP_delay=10e6, detune=0, echotype=ECHO_NONE, necho=1, double_freq=False, inj_len=10e3, **kwargs):
        ...
        super(T2Measurement_QP, self).__init__(len(delays), infos=(qubit_info,), **kwargs)
...
#            s.append(Pad(r(np.pi/2, X_AXIS), 250, PAD_LEFT))
            ...
            if self.inj_len < 20000 :
                s.append(Constant(self.inj_len, 1, chan="3m1"))
            else:
                n_10us_pulse = int(self.inj_len)/10000 - 1
                s.append(Repeat(Constant(10000, 1, chan="3m1"), n_10us_pulse))
                s.append(Constant(self.inj_len-n_10us_pulse*10000, 1, chan="3m1"))
