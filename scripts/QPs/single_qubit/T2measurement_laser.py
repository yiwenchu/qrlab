...
def analysis(meas, data=None, fig=None):
    ...
    if meas.double_freq == True:
        ...
        fig.axes[0].set_ylabel('Intensity [AU]')
        fig.axes[0].set_xlabel('Time [us]')
        fig.axes[1].plot(xs/1e3, double_sin_fit(params2, xs, ys), marker='s')
        fig.canvas.draw()
        return params3

    return params

class T2Measurement_laser(Measurement1D):

    def __init__(self, qubit_info, delays, detune=0, echotype=ECHO_NONE, necho=1, double_freq=False, laser_delay = None, laser_plen = 250, **kwargs):
        ...
        self.laser_plen = laser_plen
        self.laser_delay = laser_delay

        super(T2Measurement_laser, self).__init__(len(delays), infos=(qubit_info,), **kwargs)
    ...
    def generate(self):
        ...
        for i, dt in enumerate(self.delays):
            ...
            s.append(Constant(self.laser_plen, 1, chan = "1m2"))
...
#        s.plot_seqs(seqs)
    ...
    def analyze(self, data=None, fig=None):
        ...
        return self.fit_params['freq'].value, self.fit_params['freq'].stderr, self.fit_params['tau'].value, self.fit_params['tau'].stderr
